...
function useElementWidth<T extends HTMLElement>(ref: React.RefObject<T | null>): number {
  ...
  useLayoutEffect(() => {
    ...
    const handleFontsLoaded = () => requestAnimationFrame(updateWidth);
...
export const ScrollVelocity: React.FC<ScrollVelocityProps> = ({
  ...
  function VelocityText({
    ...
    for (let i = 0; i < (numCopies ?? 6); i++) {
      spans.push(
        <span
          className={`inline-flex flex-shrink-0 items-center ${className}`}
          key={i}
    ...
    return (
      <div className={`${parallaxClassName} relative overflow-hidden`} style={parallaxStyle}>
        <motion.div
          className={`${scrollerClassName} flex w-max whitespace-nowrap text-center`}
